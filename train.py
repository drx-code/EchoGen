...
from echogen.dataset.build import build_t2i_dataset
from echogen.utils.save_and_load import CKPTSaver, auto_resume
from echogen.utils import arg_util, misc, wandb_utils
from echogen.dino.process import preprocess_raw_image
...
def build_everything_from_args(args: arg_util.Args, saver):
    ...
    text_tokenizer, text_encoder, vae_local, gpt_uncompiled, gpt_wo_ddp, gpt_ddp, gpt_wo_ddp_ema, gpt_ddp_ema, gpt_optim, semantic_encoder, content_encoder = build_model_optimizer(args, vae_ckpt)

    # IMPORTANT: import heavy package `EchoGenTrainer` after the Dataloader object creation/iteration to avoid OOM
    from trainer import EchoGenTrainer
    # build trainer
    trainer = EchoGenTrainer(
    ...
    dist.barrier()
    return (
        text_tokenizer, text_encoder, semantic_encoder, content_encoder, trainer,
...
def build_model_optimizer(args, vae_ckpt):
    ...
    from echogen.models.echogen import EchoGen, MultipleLayers
    from echogen.models.init_param import init_weights
    from echogen.utils.amp_opt import AmpOptimizer
    from echogen.utils.lr_control import filter_params
    from echogen.utils.load import build_vae_gpt
    ...
    if args.rush_resume:
        # load pretrained echogen checkpoint
        ...
        if 'trainer' not in cpu_d:
            gpt_wo_ddp.control_load_state_dict()
        if args.use_fsdp_model_ema:
            gpt_wo_ddp_ema.load_state_dict(drop_unfit_weights(ema_state_dict), strict=False)
            if 'trainer' not in cpu_d:
                gpt_wo_ddp_ema.control_load_state_dict()
    ...
    # fliter params through bias or weight
    names, paras, para_groups = filter_params(args, gpt_ddp if args.zero else gpt_wo_ddp, ndim_dict, nowd_keys=nowd_keys)
    del ndim_dict

    param_groups_for_opt = []
    base_lr = args.tlr
    base_wd = 0
    if args.oeps: opt_eps = args.oeps
    ...
    for group in para_groups:
        ...
        opt_group = {
            'params': group['params'],
            'lr': base_lr * lr_sc,                 
            'weight_decay': base_wd * wd_sc,
        }
        if args.oeps:
            opt_group['eps'] = opt_eps

        param_groups_for_opt.append(opt_group)

    print(f'[vgpt] Optimizer param groups:')
    for i, g in enumerate(param_groups_for_opt):
        print(f"Group {i}: {len(g['params'])} params, lr={g['lr']:.2e}, wd={g['weight_decay']}")
    ...
    # Important: lr = args.tlr, weight_decay=0
    opt_kw = {}
    if args.oeps: opt_kw['eps'] = args.oeps
    # if args.oeps: opt_kw['eps'] = args.oeps
    print(f'[vgpt] optim={opt_clz}, opt_kw={opt_kw}\n')
    gpt_optim = AmpOptimizer('gpt', args.fp16, opt_clz(params=param_groups_for_opt, **opt_kw), gpt_ddp if args.zero else gpt_wo_ddp, args.r_accu, args.tclip, args.zero)
    ...
    semantic_enc_type = args.semantic_enc_type
    encoders, _, _ = load_encoders(enc_type=semantic_enc_type, device=args.device, resolution=512 if args.pn=='1M' else 256)
    semantic_encoder = encoders[0]
    semantic_encoder.to(args.device)
    semantic_encoder.eval()
    semantic_encoder.requires_grad_(False)
    [p.requires_grad_(False) for p in semantic_encoder.parameters()]
    
    content_encoder = AutoencoderKL.from_pretrained(args.content_ckpt_path, subfolder="vae", torch_dtype=torch.float32)
    content_encoder.to(args.device)
    content_encoder.eval()
    content_encoder.requires_grad_(False)
    [p.requires_grad_(False) for p in content_encoder.parameters()]
    
    return text_tokenizer, text_encoder, vae_local, gpt_uncompiled, gpt_wo_ddp, gpt_ddp, gpt_wo_ddp_ema, gpt_ddp_ema, gpt_optim, semantic_encoder, content_encoder
...
def build_dataloaders(args):
    if args.task_type == 't2i':
        dataset_train = build_t2i_dataset(
            ...
            load_vae_instead_of_image=args.load_vae_instead_of_image
        )
    else:
    ...
    # generator=None for toy dataset
...
def main_train(args: arg_util.Args):
    ...
    from trainer import EchoGenTrainer
    ret: Tuple[
        misc.TensorboardLogger, T5TokenizerFast, T5EncoderModel, EchoGenTrainer,
    ...
    logging_params_milestone: List[int] = np.linspace(1, args.ep, 10+1, dtype=int).tolist() # logging milestone: [1, 10,...,100]
    milestone_ep_feishu_log = set(seg5[:])
    vis_milestone_ep = set(seg5[:]) | set(x for x in (2, 4, 8, 16) if x <= args.ep) # {1,2,100,4,40,8,80,16,20,60}
    ...
    # build wandb logger
    if dist.is_master():
        wandb_utils.wandb.init(project=args.project_name, name=args.exp_name, config={}, dir=args.local_out_path)
    for ep in range(start_ep, args.ep):
        ...
        stats, (sec, remain_time, finish_time) = train_one_ep(
            ...
            text_tokenizer=text_tokenizer, text_encoder=text_encoder, semantic_encoder=semantic_encoder, content_encoder=content_encoder,
            trainer=trainer,
...
def train_one_ep(
    ...
    text_tokenizer: T5TokenizerFast, text_encoder: T5EncoderModel, semantic_encoder, content_encoder, trainer, logging_params_milestone, enable_timeline_sdk: bool,
):
    # IMPORTANT: import heavy packages after the Dataloader object creation/iteration to avoid OOM
    from trainer import EchoGenTrainer
    from echogen.utils.lr_control import lr_wd_annealing
    trainer: EchoGenTrainer
    
    step_cnt = 0
    header = f'[Ep]: [{ep:4d}/{args.ep}]'
    print(args.save_model_iters_freq)
    ...
    with misc.Low_GPU_usage(files=[args.log_txt_path], sleep_secs=20, verbose=True) as telling_dont_kill:
        ...
        for it, data in me.log_every(start_it, iters_train, ld_or_itrt, args.log_freq, args.log_every_iter, header):
            ...
            with maybe_record_function('before_train'):
                ...
                input_ids = tokens.input_ids.cuda(non_blocking=True) # [B, text_tokenizer.model_max_length]
                mask = tokens.attention_mask.cuda(non_blocking=True) # [B, text_tokenizer.model_max_length], 0 for padding positions, 1 for others
                text_features = text_encoder(input_ids=input_ids, attention_mask=mask)['last_hidden_state'].float() # [B, text_tokenizer.model_max_length, dim]
                
                # preprocess conditional images
                # maybe need to downsample conditional images
                cond_inp = cond_inp.cuda(non_blocking=True)
                semantic_input = preprocess_raw_image(cond_inp, enc_type=args.semantic_enc_type)
                semantic_feats = semantic_encoder.forward_features(semantic_input)

                content_feats = content_encoder.encode(cond_inp)[0].mode()

                # lens of every captions text condition
                lens: List[int] = mask.sum(dim=-1).tolist()
                # sum of lens
                cu_seqlens_k = F.pad(mask.sum(dim=-1).to(dtype=torch.int32).cumsum_(0), (1, 0))
                Ltext = max(lens) # max length of captions
                
                kv_compact = []
                for len_i, feat_i in zip(lens, text_features.unbind(0)):
                    kv_compact.append(feat_i[:len_i])
                kv_compact = torch.cat(kv_compact, dim=0) # compact text conditions: [l1+l2+...+l_bs, dim]
                ...
                # max_tlr: current max lr for params
                ...
                # stepping: optimizer step or not
                stepping = (g_it + 1) % args.ac == 0
                step_cnt += int(stepping)
            
            # g_it: global iteration; it: iteration in epoch; ep: epoch
            # stepping: stepping iteration when accumulate gradient 
            # inp: images; text_cond_tuple: text conditions
