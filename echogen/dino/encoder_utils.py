...
from echogen.dino.monov3_vit import *
    ...
    for enc_name in enc_names:
        ...
        if resolution == 512:
            if encoder_type != 'dinov2' and encoder_type != 'dinov3':
                raise NotImplementedError(
                    "Currently, we only support 512x512 experiments with DINOv2 or DINOV3 encoders."
                    )
        ...
        if encoder_type == 'mocov3':
            ...
            if 'reg' in encoder_type:
                encoder = torch.hub.load('dinov2', f'dinov2_vit{model_config}14_reg', source='local')
            else:
                encoder = torch.hub.load('dinov2', f'dinov2_vit{model_config}14', source='local')
            del encoder.head
...
if __name__ == "__main__":
    encoders, _, _ = load_encoders(enc_type = "dinov2-vit-b", device="cpu")
    dino_encoder = encoders[0]
    tensor = torch.randn((2, 3, 224, 224))
    output = dino_encoder.forward_features(tensor)
    print("Dino encode")