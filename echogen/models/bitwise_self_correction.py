...
class BitwiseSelfCorrection(object):
    ...
    def flip_requant(self, vae_scale_schedule, raw_features, device, inp_B3HW=None):
        with torch.amp.autocast('cuda', enabled = False):
            ...
            for si, (pt, ph, pw) in enumerate(vae_scale_schedule):
                ...
                # bit indices: False or True for quantized latent
                ...
                if si < self.noise_apply_layers:
                    ...
                    pred_bit_indices[mask] = 1 - pred_bit_indices[mask] # add mask
            ...
            if self.apply_spatial_patchify:
                ...
                # list: elem---bitindices with [B, h*w, C]
                gt_ms_idx_Bl = [item.reshape(B, -1, self.vae.codebook_dim) for item in gt_all_bit_indices]
            x_BLC_wo_prefix = torch.cat(x_BLC_wo_prefix, 1) # (B, L, C)
