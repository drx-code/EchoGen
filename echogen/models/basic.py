...
def apply_rotary_emb(q, k, scale_schedule, rope2d_freqs_grid, pad_to_multiplier, rope2d_normalized_by_hw, scale_ind):
    ...
    with torch.autocast(device_type=device_type, enabled=False):
        ...
        ], dim=-1) # (2, batch_size, heads, seq_len, half_head_dim, 2),
...
class SelfAttention(nn.Module):
    ...
    def kv_caching(self, enable: bool): 
        # kv caching: only used during inference
        self.caching = enable
        self.cached_k = None
        self.cached_v = None
    ...
    def forward(self, x, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        ...
        if self.using_flash: q, k, v = qkv.unbind(dim=2); L_dim = 1           
        else: q, k, v = qkv.permute(2, 0, 3, 1, 4).unbind(dim=0); L_dim = 2 
...
class CrossAttention(nn.Module):
    def __init__(
        ...
        """
        :param for_attn_pool: only used in VAR.text_proj_for_sos, learn q for text_proj_for_sos
...
class MMAttention(nn.Module):
    def __init__(
        ...
        if self.cos_attn:
            ...
            self.scale_mul_1H11 = nn.Parameter(torch.full(size=size, fill_value=4.0).log(), requires_grad=True)
            self.max_scale_mul = torch.log(torch.tensor(100)).item()
            self.control_scale_mul_1H11 = nn.Parameter(torch.full(size=size, fill_value=4.0).log(), requires_grad=True)
        else:
            self.scale = 1 / math.sqrt(self.head_dim) / self.tau
        ...
        # introduced parameters for content image features
        self.control_mat_q = nn.Linear(embed_dim, embed_dim, bias=False)
        self.control_mat_kv = nn.Linear(embed_dim, embed_dim * 2, bias=False)
        self.control_q_bias, self.control_v_bias = nn.Parameter(torch.zeros(embed_dim)), nn.Parameter(torch.zeros(embed_dim))
        self.register_buffer('control_zero_k_bias', torch.zeros(embed_dim))
        self.control_proj = nn.Linear(embed_dim, embed_dim)
        self.control_proj_drop = get_dropout_layer(proj_drop)
        # Use a zero-initialized linear layer to initially suppress the attention scores from content features to multi-scale tokens, 
        # improving gradient stability.
        self.control_attn = nn.Linear(embed_dim, embed_dim, bias=False)
    ...
    def kv_caching(self, enable: bool): 
        ...
        self.cached_kc = None
        self.cached_vc = None
        self.cached_oup_c2 = None
        self.cached_oup_c1 = None
    
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, content_feats, attn_bias_or_two_vector: Union[torch.Tensor, Tuple[torch.IntTensor, torch.IntTensor]], attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        """
            :param (fp32) x: Input tokens, shape (B, L, C). If sequence parallelism is enabled, the L dimension is partitioned across devices.
            :param (fp32) content_feats: Reference/content features, shape (B, L, C). If sequence parallelism is enabled, the L dimension is partitioned across devices.
            :param (fp32) attn_bias_or_two_vector:
                - If not using flash attention:
                    A block-wise attention bias matrix (as in the paper) that specifies the visibility pattern:
                    image tokens can attend to the content features of reference image, while content features cannot attend to image tokens.
                - If using flash attention:
                    A tuple of two 1D integer tensors: (VAR_visible_kvlen, VAR_invisible_qlen).
            :return:
                oup: Updated output for x, shape (B, L, C).
                oup_c: Updated output for c, shape (B, L, C).
        """
        # x: fp32
        B, L, C = x.shape
        ...
        if self.caching == False or self.cached_kc == None:
            L_c = content_feats.shape[-2]
            q_c = self.control_mat_q(content_feats)
            kv_c = self.control_mat_kv(content_feats)
            qkv_c = torch.cat((q_c, kv_c), dim=-1)  # BxLx3Hc
            qkv_c += torch.cat((self.control_q_bias, self.control_zero_k_bias, self.control_v_bias))
            qkv_c = qkv_c.view(B, -1, 3, self.num_heads, self.head_dim) 
            if self.using_flash: q_c, k_c, v_c = qkv_c.unbind(dim=2); L_dim = 1           # q or k or v: all are shaped in (B:batch_size, L:seq_len, H:heads, c:head_dim)
            else: q_c, k_c, v_c = qkv_c.permute(2, 0, 3, 1, 4).unbind(dim=0); L_dim = 2   # q or k or v: all are shaped in (B:batch_size, H:heads, L:seq_len, c:head_dim)

            assert self.cos_attn is True
            scale_mul_c = self.control_scale_mul_1H11.clamp_max(self.max_scale_mul).exp() # 11H1 (flash), or 1H11 (not flash)
            q_c = F.normalize(q_c, dim=-1, eps=1e-12).mul(scale_mul_c).contiguous()   # fp32
            k_c = F.normalize(k_c, dim=-1, eps=1e-12).contiguous()                  # fp32
            v_c = v_c.contiguous()         

            if self.caching: self.cached_qc, self.cached_kc, self.cached_vc = q_c.clone(), k_c.clone(), v_c.clone()
        else:
            L_c = content_feats.shape[-2]
            q_c, k_c, v_c =self.cached_qc.clone(), self.cached_kc.clone(), self.cached_vc.clone()
        
        if self.caching:    
            # kv caching: only used during inference
            len_cached = 0 if self.cached_k is None else self.cached_k.shape[L_dim]
            attn_mask = attn_bias_or_two_vector[:,:,len_cached:len_cached+L,:len_cached+L] # shape: [1,1,L,L+L_cached]
            attn_mask = F.pad(attn_mask, (0, L_c, 0, L_c), value=0) # shape: [1,1,L+L_c,L+L_cached+L_c]
            attn_mask[:,:,L:,:len_cached+L] = -torch.inf
            attn_mask = attn_mask.type_as(q).to(q.device)
        
            if self.cached_k is None: self.cached_k = k; self.cached_v = v
            elif self.using_flash: k = self.cached_k = torch.cat((self.cached_k, k), dim=L_dim); v = self.cached_v = torch.cat((self.cached_v, v), dim=L_dim)
            else: k = self.cached_k = torch.cat((self.cached_k, k), dim=L_dim); v = self.cached_v = torch.cat((self.cached_v, v), dim=L_dim)
        else:
            attn_mask = attn_bias_or_two_vector

        # multi-scale image tokens self-attention
        if self.using_flash:
            if attn_mask is not None:
                kw = dict(VAR_visible_kvlen=attn_mask[0], VAR_invisible_qlen=attn_mask[1])
            else:
                kw = dict(VAR_visible_kvlen=attn_mask[0], VAR_invisible_qlen=attn_mask[1])
            oup = flash_attn_func(q.to(v.dtype), k.to(v.dtype), v, dropout_p=0, softmax_scale=self.scale, **kw).view(B, L, C)
        else:
            if self.use_flex_attn and attn_fn is not None:
                oup = attn_fn(q, k, v, scale=self.scale).transpose(1, 2).reshape(B, -1, C)
            else:
                oup = slow_attn(query=q, key=k, value=v, scale=self.scale, attn_mask=attn_mask[:,:,:L,:-L_c], dropout_p=0).transpose(1, 2).reshape(B, -1, C)

        # attention from content features to image tokens
        oup2 = slow_attn(query=q, key=k_c, value=v_c, scale=self.scale, attn_mask=attn_mask[:,:,:L,-L_c:], dropout_p=0).transpose(1, 2).reshape(B, -1, C)
        oup = oup + self.control_attn(oup2)
        
        # content features self-attention
        if self.caching == False or self.cached_oup_c1 == None:
            oup_c = slow_attn(query=q_c, key=torch.cat((k, k_c), dim=L_dim), value=torch.cat((v, v_c), dim=L_dim), scale=self.scale, attn_mask=attn_mask[:,:,L:], dropout_p=0).transpose(1, 2).reshape(B, -1, C)
            if self.caching: self.cached_oup_c1 = oup_c.clone()
        else:
            oup_c = self.cached_oup_c1.clone()

        # projection for output
        oup = self.proj_drop(self.proj(oup))
        if self.caching == False or self.cached_oup_c2 == None:
            oup_c = self.control_proj_drop(self.control_proj(oup_c))
            if self.caching: self.cached_oup_c2 = oup_c.clone()
        else:

            oup_c = self.cached_oup_c2.clone()
        return oup, oup_c
...
class SubjectCrossAttention(nn.Module):
    def __init__(
        ...
        # introduced parameters for semantic features of reference images
        self.control_mat_kv = nn.Linear(kv_dim, embed_dim*2, bias=False)
        self.control_v_bias = nn.Parameter(torch.zeros(embed_dim))
        ...
        self.caching = False
        self.cached_kv_compact = None
        self.cached_control_kv_compact = None        
    ...
    def forward(self, q, ca_kv, semantic_feats):
        """
        :param q: shaped as (batch, seq_len, Q_dim)
        :param ca_kv: contains several vectors, each of which is shaped as (len_i, KV_dim). We have [len_1xKV_dim, len_2xKV_dim, len_3xKV_dim, ...] and lens == [len_1, len_2, len_3, ...]
            - kv_compact: shaped as (sum(lens), KV_dim)
            - cu_seqlens_k: cumulated sum of lens
            - max_seqlen_k: int, max(lens)
        :param semantic_feats: the semantic features of reference images, shaped as (batch, semantic_len, semantic_dim)
        NOTE: seq_len (num of Qs) can reach 10k;  but len_i (num of KVs) must <= 256
        :return: shaped as (batch, seq_len, Q_dim)
        """
        # q processing
        ...
        if self.caching == False or self.cached_control_kv_compact == None:
            # for semantic features
            bs, semantic_len, _ = semantic_feats.shape[:] 
            semantic_feats = semantic_feats.reshape(-1, semantic_feats.shape[-1])
            semantic_N = semantic_feats.shape[0]
            semantic_kv_compact = F.linear(semantic_feats, weight=self.control_mat_kv.weight, bias=torch.cat((self.zero_k_bias, self.control_v_bias))).view(semantic_N, 2, self.num_heads, self.head_dim) # NC => N2Hc
            ...
            if self.cos_attn:
                ...
                semantic_k, semantic_v = semantic_kv_compact.unbind(dim=1)
                semantic_k = F.normalize(semantic_k, dim=-1)
                semantic_kv_compact = torch.stack((semantic_k, semantic_v), dim=1)
        
            if self.caching:
                self.cached_control_kv_compact = semantic_kv_compact.clone()
                self.cached_kv_compact = kv_compact.clone()
        else:
            _, cu_seqlens_k, max_seqlen_k = ca_kv
            kv_compact = self.cached_kv_compact.clone()

            bs, semantic_len, _ = semantic_feats.shape[:] 
            semantic_kv_compact = self.cached_control_kv_compact.clone()

        # merge for the final compact of kv
        final_compact = []
        for i in range(len(cu_seqlens_k)-1):
            per_kv_compact = kv_compact[cu_seqlens_k[i]:cu_seqlens_k[i+1]]

            per_semantic_compact = semantic_kv_compact[semantic_len*i:semantic_len*(i+1)]
            final_compact.append(torch.cat([per_kv_compact, per_semantic_compact], dim=0))
        final_compact = torch.cat(final_compact, dim=0)

        cu_seqlens_k = cu_seqlens_k+ torch.arange(0, semantic_len * (B+1), semantic_len, dtype=torch.int32, device=cu_seqlens_k.device)
        max_seqlen_k = max_seqlen_k + semantic_len

        q_compact = q_compact.contiguous()
        final_compact = final_compact.contiguous()
        
        cu_seqlens_q = torch.arange(0, Lq * (B+1), Lq, dtype=torch.int32, device=q_compact.device)
        if q_compact.dtype == torch.float32:
            oup = flash_attn_varlen_kvpacked_func(q=q_compact.to(dtype=torch.bfloat16), kv=final_compact.to(dtype=torch.bfloat16), cu_seqlens_q=cu_seqlens_q, cu_seqlens_k=cu_seqlens_k, max_seqlen_q=Lq, max_seqlen_k=max_seqlen_k, dropout_p=0, softmax_scale=self.scale).reshape(B, Lq, -1)
            oup = oup.float()
        else:
            oup = flash_attn_varlen_kvpacked_func(q=q_compact, kv=final_compact, cu_seqlens_q=cu_seqlens_q, cu_seqlens_k=cu_seqlens_k, max_seqlen_q=Lq, max_seqlen_k=max_seqlen_k, dropout_p=0, softmax_scale=self.scale).reshape(B, Lq, -1)
...
class EchoGenBlock(nn.Module):
    def __init__(
        ...
        super(EchoGenBlock, self).__init__()
        self.C, self.D = embed_dim, cond_dim
        self.drop_path_rate = drop_path
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        ...
        # cross-attention between multi-scale image token sequence and (text-condition, semantic condition)
        self.ca = SubjectCrossAttention(embed_dim=embed_dim, kv_dim=kv_dim, num_heads=num_heads, proj_drop=drop, cos_attn=cos_attn)
        
        # cross-attention between content condition and text-condition
        self.control_ca = CrossAttention(embed_dim=embed_dim, kv_dim=kv_dim, num_heads=num_heads, proj_drop=drop, cos_attn=cos_attn)
        
        self.using_swiglu = swiglu
        self.ffn = (FFNSwiGLU if swiglu else FFN)(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio / 256) * 256, drop=drop, fused_mlp=fused_mlp)
        
        self.ln_wo_grad = norm_layer(embed_dim, elementwise_affine=False) # LayerNorm
        self.fused_norm_func = fused_norm_func
        self.norm_eps = norm_layer.keywords.get('eps', 1e-6)

        self.control_fused_norm_func = fused_norm_func
        self.ca_norm = norm_layer(embed_dim, elementwise_affine=True)
        self.control_ca_norm = norm_layer(embed_dim, elementwise_affine=True)
        
        self.shared_aln = shared_aln
        if self.shared_aln:
            # nn.Parameters for sos
            self.ada_gss = nn.Parameter(torch.randn(1, 1, 6, embed_dim) / embed_dim**0.5)
            self.control_ada_gss = nn.Parameter(torch.zeros(1, 1, 6, embed_dim) / embed_dim**0.5)
        else:
            lin = nn.Linear(cond_dim, 6*embed_dim)
            self.ada_lin = nn.Sequential(nn.SiLU(inplace=False), lin) if act else nn.Sequential(lin)
        ...
        if cross_attn_layer_scale >= 0:
            self.control_ca_gamma = nn.Parameter(cross_attn_layer_scale * torch.ones(embed_dim), requires_grad=True)
        else:
            self.control_ca_gamma = 1

        self.checkpointing_sa_only = checkpointing_sa_only
        
        # new-introduced ffn for content image condition
        self.control_drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.control_ffn = (FFNSwiGLU if swiglu else FFN)(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio / 256) * 256, drop=drop, fused_mlp=fused_mlp)

        self.caching = False

    def kv_caching(self, enable: bool): # kv caching: only used during inference
        self.caching = enable
        self.cached_gamma1_content, self.cached_gamma2_content = None, None
        self.cached_scale1_content, self.cached_scale2_content = None, None
        self.cached_shift1_content, self.cached_shift2_content = None, None
        self.cached_conent_sa = None
        self.cached_content_feats = None
        self.cached_content_feats2 = None
        self.cached_content_feats3 = None

    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, cond_BD_content, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, semantic_feats=None, content_feats=None):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        with torch.amp.autocast("cuda", enabled=False):    # disable half precision
            if self.shared_aln: #                  (1, 1, 6, C)  + (B, 1, 6, C)
                gamma1, gamma2, scale1, scale2, shift1, shift2 = (self.ada_gss + cond_BD).unbind(2) # 116C + B16C =unbind(2)=> 6 B1C
                if self.caching == False or self.cached_gamma1_content==None:
                    gamma1_content, gamma2_content, scale1_content, scale2_content, shift1_content, shift2_content = (self.control_ada_gss + cond_BD_content).unbind(2)
                    if self.caching: 
                        self.cached_gamma1_content, self.cached_gamma2_content = gamma1_content.clone(), gamma2_content.clone()
                        self.cached_scale1_content, self.cached_scale2_content = scale1_content.clone(), scale2_content.clone()
                        self.cached_shift1_content, self.cached_shift2_content = shift1_content.clone(), shift2_content.clone()
                else:
                    gamma1_content, gamma2_content = self.cached_gamma1_content.clone(), self.cached_gamma2_content.clone()
                    scale1_content, scale2_content = self.cached_scale1_content.clone(), self.cached_scale2_content.clone()
                    shift1_content, shift2_content = self.cached_shift1_content.clone(), self.cached_shift2_content.clone()
            else:
                gamma1, gamma2, scale1, scale2, shift1, shift2 = self.ada_lin(cond_BD).view(-1, 1, 6, self.C).unbind(2)
                gamma1_content, gamma2_content, scale1_content, scale2_content, shift1_content, shift2_content = self.ada_lin(cond_BD_cond).view(-1, 1, 6, self.C).unbind(2)
        
        x_sa = self.fused_norm_func(C=self.C, eps=self.norm_eps, x=x, scale=scale1, shift=shift1)
        if self.caching == False or self.cached_conent_sa == None:
            content_feats_sa = self.control_fused_norm_func(C=self.C, eps=self.norm_eps, x=content_feats, scale=scale1_content, shift=shift1_content)
            if self.caching: self.cached_conent_sa = content_feats_sa.clone()
        else:
            content_feats_sa = self.cached_conent_sa.clone()

        if self.checkpointing_sa_only and self.training:
            x_sa, content_feats_sa = checkpoint(self.sa, x_sa, content_feats_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, use_reentrant=False)
        else:
            x_sa, content_feats_sa = self.sa(x_sa, content_feats_sa, attn_bias_or_two_vector, attn_fn, scale_schedule, rope2d_freqs_grid, scale_ind=scale_ind)
        
        x = x + self.drop_path(x_sa.mul_(gamma1))
        if self.caching == False or self.cached_content_feats == None:
            content_feats = content_feats + self.control_drop_path(content_feats_sa.mul_(gamma1_content))
            if self.caching:
                self.cached_content_feats = content_feats.clone()
        else:
            content_feats = self.cached_content_feats.clone()

        x = x + self.ca(self.ca_norm(x), ca_kv, semantic_feats).float().mul_(self.ca_gamma)
        if self.caching == False or self.cached_content_feats2 == None:
            content_feats = content_feats + self.control_ca(self.control_ca_norm(content_feats), ca_kv).float().mul_(self.control_ca_gamma)
            if self.caching: self.cached_content_feats2 = content_feats.clone()
        else:
            content_feats = self.cached_content_feats2.clone()
        ...
        if self.caching == False or self.cached_content_feats3 == None:
            content_feats = content_feats + self.control_drop_path(self.control_ffn(self.control_fused_norm_func(C=self.C, eps=self.norm_eps, x=content_feats, scale=scale2_content, shift=shift2_content)).mul(gamma2_content))
            if self.caching: self.cached_content_feats3 = content_feats.clone()
        else:
            content_feats = self.cached_content_feats3.clone()     
        return x, content_feats
