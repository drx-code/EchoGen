...
class MultiScaleBSQ(Module):
    ...
    def __init__(
        ...
        *,
        ...
        random_flip = False, # random flip when scale level < max_flip_lvl
        flip_prob = 0.5,
        flip_mode = "stochastic", # "stochastic", "deterministic"
        max_flip_lvl = 1, # max_flip_scale_level
    ...
    def forward(
        ...
        scale_num = len(scale_schedule) # number of scales

        with autocast('cuda', enabled = False):
            for si, (pt, ph, pw) in enumerate(scale_schedule):
                out_fact = max(0.1, out_fact) if self.use_decay_factor else init_out_fact # 1.0

                if (pt, ph, pw) != (T, H, W):
                    interpolate_residual = F.interpolate(residual, size=(pt, ph, pw), mode=self.z_interplote_down) # shape: (pt, ph, pw) for current residual
                else:
                ...
                # when self.use_stochastic_depth== True, drop output
                if self.training and self.use_stochastic_depth and random.random() < self.drop_rate:
                    ...
                    # flip when self.random_flip or random_flip_1lvl
                ...
                if (pt, ph, pw) != (T, H, W):
                    quantized = F.interpolate(quantized, size=(T, H, W), mode=self.z_interplote_up).contiguous() # shape: (T, H, W) for current residual
                
                if self.remove_residual_detach:
                    residual = residual - quantized
                else:
                    residual = residual - quantized.detach()
                quantized_out = quantized_out + quantized # F_k in paper, all quatized summation

                # add bit indices and loss
                all_bit_indices.append(bit_indices)
                all_losses.append(loss)

                # add var input: \tilewide{F}_k
                if si != scale_num - 1:
        ...
        # quantized_out tenosr; all indices; all bit indices; []; loss; var_inputs
...
class BSQ(Module):
    ...
    def indices_to_codes(
        ...
        if not self.keep_num_codebooks_dim:
            if label_type == 'int_label':
                ...
                indices = indices.unsqueeze(-2) # ad channels for codebook
    ...
    def quantize_new(self, z):
        ...
        # quantized scale, +/-  1. / (self.codebook_dims ** 0.5) (codebook_scale)
        # bit_indices = (quantized > 0).int(), +1 for 1. / (self.codebook_dims ** 0.5), 0 for -1. / (self.codebook_dims ** 0.5)
    ...
    def forward(
        ...
        x = rearrange(x, 'b n (c d) -> b n c d', c = self.num_codebooks) # 1 by default

        x = l2norm(x) # Normalizaton
        ...
        # x: quantized tensor, indices: None, bit_indices: bit indices, shape: B, T, H, W, C
