...
from echogen.utils.dynamic_resolution import dynamic_resolution_h_w, get_h_div_w_template2indices, h_div_w_templates
from echogen.utils.large_file_util import get_part_jsonls, split_large_txt_files
...
def transform(pil_img, tgt_h, tgt_w):
    ...
    # resize with  PIMage.LANZOS
    pil_img = pil_img.resize((resized_width, resized_height), resample=PImage.LANCZOS)
    # center crop the center out
...
class T2IIterableDataset(IterableDataset):
    def __init__(
        ...
        **kwargs,
        ...
        # samples_div_gpus_workers_batchsize_2batches: sample batches
        ...
        self.split_meta_files() # add key'part_filepaths', value: path of config file
        self.seed = seed
    ...
    def set_h_div_w_template2generator(self,):
        ...
        # filename: "h_div_w"_"num_samples"
        ...
        for filepath in sorted(glob.glob(osp.join(self.meta_folder, '*.jsonl'))):
            ...
            # h_div_w_template2generator: "keys"--h_div_w_template; "attributes": 'filepath' (json file); 
            #   'num_of_samples' (sample numbers); 'num_of_batches' (batches)
            h_div_w_template2generator[h_div_w_template] = {
                'filepath': filepath,
                'num_of_samples': num_of_samples,
                'num_of_batches': num_of_batches,
            }
            samples_div_gpus_workers_batchsize_2batches += num_of_batches # number of batches
    ...
    def set_generator(self, ):
        """Set generator, mainly random seed about epoch"""
    ...
    def __iter__(self):
        ...
        for h_div_w_template, generator_info in self.h_div_w_template2generator.items():
            ...
            # Default: generator_info['num_of_batches'] * self.batch_size is less than h_div_w_buffer_size (self.buffer_size)
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            while len(batch_data) < self.batch_size:
                try:
                    ...
                    if ret and self.load_vae_instead_of_image == False:
    ...
    def infinite_next(self, generator_info):
        try:
            ...
            generator_info['record_iterator'] = open(part_filepath, 'r', encoding='utf-8') # get record_iteration
            ...
            generator_info['sub_iterator'] = itertools.islice(generator_info['record_iterator'], self.worker_id, part_num_of_samples, self.dataloader_workers) # Load data
            return next(generator_info['sub_iterator'])
...
class SubjectDrivenT2IIterableDataset(T2IIterableDataset):
    def __init__(
        self, 
        meta_folder: str, 
        max_caption_len=512, 
        ...
        **kwargs,
        ...
        super().__init__(meta_folder, max_caption_len, short_prob, load_vae_instead_of_image, buffersize, seed, pn, online_t5, batch_size, num_replicas, 
                         rank, dataloader_workers, dynamic_resolution_across_gpus, enable_dynamic_length_prompt)
    ...
    def __iter__(self):
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            captions = [item[0] for item in batch_data]
            images = torch.stack([item[1] for item in batch_data])
            cond_img = torch.stack([item[2] for item in batch_data])
            yield (images, captions, cond_img)
            del batch_data
            del images
            del captions
            del cond_img
    ...
    def prepare_model_input(self, data_item) -> Tuple:
        output_image, cond_image, h_div_w = data_item['output_image'], data_item['cond_image'], data_item['h_div_w']
        short_text_input, long_text_input = data_item['long_caption'], data_item['long_caption']
        long_text_type = data_item.get('long_caption_type', 'user_prompt')
        text_input = self.get_text_input(long_text_input, short_text_input, long_text_type)
        text_input = process_short_text(text_input)
        ...
        try:
            if self.load_vae_instead_of_image:
                img_B3HW = None
                vae_path = self.get_vae_path(output_image)
                with open(vae_path, 'rb') as f:
                    gt_ms_idx_Bl = pickle.load(f)
            else:
                gt_ms_idx_Bl = None
                ...
                with open(cond_image, 'rb') as f:
                    cond_img: PImage.Image = PImage.open(f)
                    cond_img = cond_img.convert('RGB')
                    tgt_h_cond = 512 if self.pn == '1M' else 256
                    cond_img_B3HW = transform(cond_img, tgt_h=tgt_h_cond, tgt_w=tgt_h_cond) # DiNO v2 input

            if not self.online_t5:
                short_t5_path, long_t5_path = self.get_t5_path(output_image)
                if self.epoch_global_worker_generator.random() <= self.short_prob:
                    t5_path = short_t5_path
                else:
                    t5_path = long_t5_path
        ...
        if self.load_vae_instead_of_image:
            return True, (text_input, *gt_ms_idx_Bl, cond_img_B3HW)
        else:
            return True, (text_input, img_B3HW, cond_img_B3HW)

class SubjectDrivenT2IIterableDatasetPath(SubjectDrivenT2IIterableDataset):
    def __init__(
        self, 
        meta_folder: str, 
        max_caption_len=512, 
    ...
    def __iter__(self):
        ...
        for _ in range(self.samples_div_gpus_workers_batchsize_2batches):
            ...
            images = torch.stack([item[1] for item in batch_data])
            cond_img = torch.stack([item[2] for item in batch_data])
            image_path = [item[3] for item in batch_data]
            cond_path = [item[4] for item in batch_data]
            yield (images, captions, cond_img, image_path, cond_path)
            del batch_data
            del images
            del captions
            del cond_img
    ...
    def prepare_model_input(self, data_item) -> Tuple:
        ...
        try:
            if self.load_vae_instead_of_image:
                ...
                with open(cond_image, 'rb') as f:
                    cond_img: PImage.Image = PImage.open(f)
                    cond_img = cond_img.convert('RGB')
                    cond_img_B3HW = transform(cond_img, tgt_h=256, tgt_w=256) # DiNO v2 input
        ...
        if self.load_vae_instead_of_image:
            return True, (text_input, *gt_ms_idx_Bl, cond_img_B3HW, output_image, cond_image)
        else:
            return True, (text_input, img_B3HW, cond_img_B3HW, output_image, cond_image)
...
if __name__ == '__main__':
    ...
    dataset = SubjectDrivenT2IIterableDatasetPath(
        args=None, 
        meta_folder='unodataset_filtered',
        data_load_reso=None, 
        max_caption_len=512, 
        short_prob=1.0, 
        load_vae_instead_of_image=False,
    ...
    #         # print(data[0].shape)
    #         # import torchvision
    #         # torchvision.utils.save_image((data[2] + 1.) / 2., "input_test.png")
