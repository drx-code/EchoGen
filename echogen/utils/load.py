...
from echogen.models.ema import get_ema_model
from echogen.utils import arg_util, misc
from echogen.utils.misc import os_system
...
def build_vae_gpt(args: arg_util.Args, vae_st: dict, skip_gpt: bool, force_flash=False, device='cuda'):
    ...
    gpt_kw = dict(
        ...
        image_cond_droprate=args.image_cond_droprate,
        content_feats_len=args.content_feats_len,
        content_feats_dim=args.content_feats_dim,
    )
    if args.dp >= 0: gpt_kw['drop_path_rate'] = args.dp # -1
    if args.hd > 0: gpt_kw['num_heads'] = args.hd # hd=0 by default
    
    print(f'[create gpt_wo_ddp] constructor kw={gpt_kw}\n')
    gpt_kw['vae_local'] = vae_local
    
    model_str = args.model.replace('vgpt', 'echogen')   # legacy
    print(f"{model_str=}")
    ...
    gpt_wo_ddp = create_model(model_str, **gpt_kw)
