...
def filter_params(args, model, ndim_dict, nowd_keys=(), lr_scale=0.0, subject_control=True) -> Tuple[
    List[str], List[torch.nn.Parameter], List[Dict[str, Union[torch.nn.Parameter, float]]]
]:
    ##############################
    # wd_sc: weight decay for parameters
    # lr_sc: actual_lr = cur_lr * lr_sc
    # specific in lr_wd_annealing function
    ##############################
    ...
    names_no_grad = []
    count, numel = 0, 0
    # decay_lrs = ['sa.control_attn', 'sa.control_proj', 'sa.control_proj_drop', 'sa.control_mat_q', 'sa.control_mat_kv', 'sa.control_q_bias', 'sa.control_v_bias', 'control_ffn', 'control_ca']
    decay_lrs = ['sa']
    for name, para in model.named_parameters():
        ...
        if with_lr_scale:
            ...
            dbg_base = f'[layer {layer_id}][sc = {lr_scale} ** {scale_exp}]'
        else:
            cur_lr_sc = 1.
            dbg_base = f'[no scale]'

        is_refer_param = False
        for decay_lr in decay_lrs:
            if decay_lr in name:
                is_refer_param = True
                break
        if is_refer_param:
