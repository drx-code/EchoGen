...
class Args(Tap):
    ...
    fuse: bool = True                  # whether to use fused mlp
    fused_norm: bool = True            # whether to use fused norm
    ...
    norm_eps: float = 1e-6              # norm eps for echogen
    ...
    load_vae_instead_of_image: int = 0

    ############################ Parameters for Subject-driven generation ############################ 
    semantic_feats_len: int = 1025
    semantic_feats_dim: int = 768        # 1024 for large
    image_cond_droprate: float = 0.1
    semantic_enc_type: str = 'dinov2-vit-b'      # 'dinov2-vit-l', 'dinov3-vit-l', 'dinov3-vit-b'
    content_feats_len: int = 4096
    content_feats_dim: int = 16
    content_ckpt_path: str = "pretrained"
...
def init_dist_and_get_args():
    ...
    args.sche = args.sche or ('lin0' if args.gpt_training else 'cos') # 'lin0' in gpt_training
    if args.wp == 0:
    ...
    args.ada = args.ada or ('0.9_0.96' if args.gpt_training else '0.5_0.9') # optimizer parameters
    args.dada = args.dada or args.ada
    args.opt = args.opt.lower().strip()
    
    if args.lbs:
        # bs is not decided by number of gpus
        bs_per_gpu = args.lbs / args.ac
    else:
        bs_per_gpu = args.bs / args.ac / dist.get_world_size()
    bs_per_gpu = round(bs_per_gpu)
    args.batch_size = bs_per_gpu # batch size per gpu 
    args.bs = args.glb_batch_size = args.batch_size * dist.get_world_size() # batch size all gpu s
    ...
    if args.gpt_training:
        ...
        from echogen.models import alias_dict, alias_dict_inv
        if args.model in alias_dict:
            ...
            args.model = f'echogen_{args.model}'
