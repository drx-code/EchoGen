...
from echogen.models.echogen import EchoGen
from echogen.models.basic import *
import PIL.Image as PImage
from torchvision.transforms.functional import to_tensor
from echogen.utils.dynamic_resolution import dynamic_resolution_h_w
from echogen.dino.process import preprocess_raw_image
from echogen.dino.encoder_utils import load_encoders
from tools.extract_subject import SubjectSegmentation
...
def encode_img_semantic(semantic_encoder, cond_inp, enc_type='dinov2-vit-b'):
    cond_inp = cond_inp.cuda(non_blocking=True)
    semantic_input = preprocess_raw_image(cond_inp, enc_type=enc_type)
    semantic_feats = semantic_encoder.forward_features(semantic_input)
    return semantic_feats

def encode_img_content(content_encoder, cond_inp):
    cond_inp = cond_inp.cuda(non_blocking=True)
    content_feats = content_encoder.encode(cond_inp)[0].sample()
...
def gen_one_img(
    ...
    cfg_i_list=None,
    semantic_enc_type='dinov2-vit-b',
    tgt_h = 256,
    tgt_w = 256,
):
    if not isinstance(cfg_t_list, list):
        cfg_t_list = [cfg_t_list] * len(scale_schedule)
    if cfg_i_list is not None and not isinstance(cfg_i_list, list):
        cfg_i_list = [cfg_i_list] * len(scale_schedule)
    ...
    with open(control_img, 'rb') as f:
        control_img: PImage.Image = PImage.open(f)
    ...
    with torch.amp.autocast("cuda", enabled=False):
        for i in range(len(control_img)):
            extracted = SubjectSegmentation(control_img[i], text_prompt=prompt[i])
            cond_image.append(Image.fromarray(extracted))

    cond_img_B3HW = [transform(i, tgt_h=tgt_h, tgt_w=tgt_w) for i in cond_image]# DiNO v2 input
    cond_img_B3HW = torch.concat(cond_img_B3HW, dim=0)

    cond_img_B3HW = cond_img_B3HW.cuda(non_blocking=True).unsqueeze(dim=0)
    cond_img_B3HW = cond_img_B3HW.repeat(1, 1, 1, 1)

    semantic_feats = encode_img_semantic(semantic_encoder, cond_img_B3HW, enc_type=semantic_enc_type)
    content_feats = encode_img_content(content_encoder, cond_img_B3HW)

    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        _, _, img_list = echogen_test.autoregressive_infer_cfg(
            vae=vae,
            scale_schedule=scale_schedule,
            label_B_or_BLT=text_cond_tuple, g_seed=g_seed,
            B=1, negative_label_B_or_BLT=negative_label_B_or_BLT, force_gt_Bhw=None,
            cfg_sc=cfg_sc, cfg_t_list=cfg_t_list, cfg_i_list=cfg_i_list, tau_list=tau_list, top_k=top_k, top_p=top_p,
...
def save_slim_model(echogen_model_path, save_file=None, device='cpu', key='gpt_fsdp'):
    print('[Save slim model]')
    full_ckpt = torch.load(echogen_model_path, map_location=device)
    echogen_slim = full_ckpt['trainer'][key]
    # ema_state_dict = cpu_d['trainer'].get('gpt_ema_fsdp', state_dict)
    if not save_file:
        save_file = osp.splitext(echogen_model_path)[0] + '-slim.pth'
    print(f'Save to {save_file}')
    torch.save(echogen_slim, save_file)
...
def load_echogen(
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        ...
        print(f'[you selected EchoGen with {model_kwargs=}]')

        if bf16:
            for block in echogen_test.unregistered_blocks:
                block.bfloat16()

        echogen_test.eval()
        echogen_test.requires_grad_(False)
        echogen_test.cuda()
        torch.cuda.empty_cache()

        print(f'[Load EchoGen weights]')
        if checkpoint_type == 'torch':
            state_dict = torch.load(model_path, map_location=device, weights_only=False)
            if 'trainer' in state_dict.keys():
                state_dict = state_dict['trainer']['gpt_fsdp']
            echogen_test.load_state_dict(state_dict)
        elif checkpoint_type == 'torch_shard':
            from transformers.modeling_utils import load_sharded_checkpoint
            load_sharded_checkpoint(echogen_test, model_path, strict=False)
        echogen_test.rng = torch.Generator(device=device)
        return echogen_test
...
def load_visual_tokenizer(args):
    ...
    if args.vae_type in [14,16,18,20,24,32,64]:
        from echogen.models.bsq_vae.vae import vae_model
...
def load_transformer(vae, args):
    ...
    echogen = load_echogen(
        ...
        content_feats_len=args.content_feats_len,
        content_feats_dim=args.content_feats_dim,
        semantic_feats_len=args.semantic_feats_len,
        semantic_feats_dim=args.semantic_feats_dim,        
    )
    return echogen

def add_common_arguments(parser):
    parser.add_argument('--cfg_t', type=str, default='3')
    parser.add_argument('--cfg_i', type=str, default='3')
    ...
    parser.add_argument('--model_type', type=str, default='echogen_2b')
    ...
    parser.add_argument('--control_img', type=str, default="")
    parser.add_argument('--semantic_enc_type', type=str, default="dinov2-vit-b")
    parser.add_argument('--content_feats_len', type=int, default=1024)
    parser.add_argument('--content_feats_dim', type=int, default=16)
    parser.add_argument('--semantic_feats_len', type=int, default=257)
    parser.add_argument('--semantic_feats_dim', type=int, default=768)
    parser.add_argument('--content_ckpt_path', type=str, default='flux')
...
if __name__ == '__main__':
    ...
    # parse cfg
    args.cfg_t = list(map(float, args.cfg_t.split(',')))
    if len(args.cfg_t) == 1:
        args.cfg_t = args.cfg_t[0]
    args.cfg_i = list(map(float, args.cfg_i.split(',')))
    if len(args.cfg_i) == 1:
        args.cfg_i = args.cfg_i[0]
    ...
    # load echogen
    echogen = load_transformer(vae, args)

    # load dino encoder
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    encoders, _, _ = load_encoders(enc_type = args.semantic_enc_type, device=device, resolution=512 if args.pn=='1M' else 256)
    ...
    with torch.amp.autocast("cuda", enabled=True, dtype=torch.bfloat16):
        with torch.no_grad():
            generated_image = gen_one_img(
                ...
                gt_leak=0,
                gt_ls_Bl=None,
                cfg_t_list=args.cfg_t,
                cfg_i_list=args.cfg_i,
                tau_list=args.tau,
                scale_schedule=scale_schedule,
                ...
                control_img=args.control_img,
                semantic_encoder=semantic_encoder,
                content_encoder=content_encoder,
                semantic_enc_type=args.semantic_enc_type,
                tgt_h=512 if args.pn == '1M' else 256,
                tgt_w=512 if args.pn == '1M' else 256,
    )
