...
from echogen.models import EchoGen
from echogen.models.ema import update_ema
from echogen.models.bitwise_self_correction import BitwiseSelfCorrection
...
class EchoGenTrainer(object):
    def __init__(
        self, is_visualizer: bool, device, raw_scale_schedule: Tuple[int, ...], resos: Tuple[int, ...],
        vae_local, gpt_wo_ddp: EchoGen, gpt: DDP, ema_ratio: float, max_it: int,
        ...
        super(EchoGenTrainer, self).__init__()
        ...
        self.gpt_wo_ddp: Union[EchoGen, torch._dynamo.eval_frame.OptimizedModule] = gpt_wo_ddp  # after torch.compile
    ...
    def train_step(
        self, ep: int, it: int, g_it: int, semantic_feats: dict, content_feats: torch.Tensor, stepping: bool, clip_decay_ratio: float, metric_lg: misc.MetricLogger, logging_params: bool,
        ...
        # search current spatial scheduler decided by the resolution 
        ...
        with self.gpt_opt.amp_ctx:
            if args.load_vae_instead_of_image == False:
                ...
                raw_features = inp_B3HW
            
            # x_BLC_wo_prefix: concatented \tildewide{F}_k for var input, shape: [B, sum(h_i*w_i), C]
            # gt_ms_idx_Bl: list for every scale, bit, shape[B, h*w, C]
            x_BLC_wo_prefix, gt_ms_idx_Bl = self.bitwise_self_correction.flip_requant(vae_scale_schedule, raw_features, device, inp_B3HW=None)
            ...
            self.gpt_wo_ddp.forward  
            logits_BLV = self.gpt(text_cond_tuple, x_BLC_wo_prefix, scale_schedule=scale_schedule[:training_scales], semantic_feats=semantic_feats, content_feats=content_feats) # [bs, 1*1+...+64*64, vocab_size or log2(vocab_size)*2]
        ...
        if metric_lg.log_every_iter or it == 0 or it in metric_lg.log_iters:
            ...
            res_bit_acc = bitwise_acc.mean(-1).mean(0) # mean cross (B,L,C)
            # all channels prediction accuarcy: if for one latent(1,1,C), every bit is correct , then 1
            ...
            # 'tnm': grad_norm_t; 'lm': Loss mean; 'lt': Loss in last scale, "Accm": accuracy per bit; Acct: accuracy in last scale 
    ...
    def __repr__(self):
        return (
            ...
            f'[VGPTTr.structure]: {super(EchoGenTrainer, self).__repr__().replace(EchoGenTrainer.__name__, "")}'
        )
